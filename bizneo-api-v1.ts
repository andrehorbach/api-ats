...
async function fetchJobs(companyId: number):  Promise<any[] | null> {
...
async function fetchCandidates(job: number, companyId: number): Promise<any[] | null> {
  ...
  do {
    try {
      console.log(`Retrieving applications from job ${job} and page ${page}...`);
...
(async () => {
    ...
    const jobs = companyId ? await fetchJobs(companyId) : null;
